...
pub async fn unsub(user_id: tbot::types::chat::Id, cmd: Arc<Command<Text>>) -> Result<(), tbot::errors::MethodCall> {
    ...
    let mut found = false;
    let mut file = OpenOptions::new().read(true).open("./feeds.txt").unwrap();  // Open file in read-only mode. 
    ...
    if found {      // If URL was found, re-write a file without the specified URL. 
        file = OpenOptions::new().write(true).truncate(true).open("./feeds.txt").unwrap();
